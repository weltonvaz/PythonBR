...
#example 2.6.2 - Dados das horas de sol em Oxford, de 1929 até 2009 (linhas)
# e de janeiro a dezembro (colunas), usando operações com listas,
...
#tarefa 1: Compute the average number of sun hours for each month during
#the total data period (1929–2009).
m=12                        #no of months
monthly_mean = [0]*m        # Criate list with 12 elements (each element is a moth in a year)
for j in range(m):          # In lists we first declare the number of columns
    s = 0                   # sum
    n = 81                  # number of years, or number of lines, (2009-1929+1)
    for i in range(n):
        s += data[i][j]
    monthly_mean[j] = s/n
...
#tarefa 2: Which month has the best weather according to the means found in
#the preceding task?

max_value = max(monthly_mean)               #find max value in monthly_mean list
position = monthly_mean.index(max_value)    #find the position (index) of max_value in monthly_mean list
month = month_names[position]               #find the corresponding month of position (index) in month_name list
print '%s has best weather with %.1f sun hours on average' % \
(month, max_value)

#tarefa 3: For each decade, 1930–1939, 1940–1949 , . . . , 2000–2009, compute the
#average number of sun hours per day in January and December. For
#example, use December 1949, January 1950 , . . . , December 1958,
#and January 1959 as data for the decade 1950–1959. Are there any
#noticeable differences between the decades?
...
for k in range(0, 81, 10):
    ...
    for j in range(k, k+10):
        print data[j][Dec_index] + data[j+1][Jan_index]
        s += data[j][Dec_index] + data[j+1][Jan_index]
    decade_mean.append(s/(10.*2*30))
    print 'a média é: %.1f' %(s/(10.*2  *30))
